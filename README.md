...
## Шаг 5:
- [ ] Добавь повторяющуюся задачу с запуском по расписанию.
- [ ] Добавь возможность запускать ту же задачу вручную.
- [ ] Настрой запуск Lighthouse CLI и генерацию отчета.
- [ ] Убедись, что отчет сохраняется и его можно посмотреть утром.
- [ ] По итогам отчета фиксируй, какие правки нужны в проекте.
